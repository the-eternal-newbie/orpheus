...
class SpotifyService:
    def __init__(self, verbose=False, **kwargs: Any):
        self.log = RadLogger(
            name="SpotifyService",
            show_timestamp=True,
            show_emoji=True,
            colored=True,
            log_file="spotify_service",
        )
        self.filesmanager = FilesManager(verbose=verbose)
    ...
    def __save_tracks(self, tracks: list[dict], formats=("csv", "json")) -> tuple[str | None, str | None]:
        if self.verbose:
        ...
        json_file = (
            self.filesmanager.write_to_file(tracks, "spotify_library", ext="json")
            if "json" in formats
            else None
        )
        time.sleep(1)
        ...
        if self.verbose:
            ...
            self.log.info(f"Saved {len(tracks)} tracks")
            self.log.end_section()
    ...
    def execute(self):
        try:
            ...
            self.log.stop_spinner(
                f"Authenticated as: {user['display_name']}", success=True
            )
