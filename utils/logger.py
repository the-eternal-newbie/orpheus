...
class RadLogger:
    ASCII_ART_FILE_MAP = {"logo": "./ascii/logo.txt", "generic": "./ascii/generic.txt"}
    ...
    def __init__(
        ...
        """
        self.filesmanager = FilesManager(verbose=True)
        self.name = name
        ...
        if self.log_file:
            self.filesmanager.write_to_file(
                f"\n{'='*50}\n"
                + f"Session started: {datetime.now()}\n"
                + f"{'='*50}\n",
    ...
    def __format_message(self, message: str, level: LogLevel) -> str:
        ...
        if timestamp:
            parts.append(
                f"{Fore.LIGHTBLACK_EX if self.colored else ''}{timestamp}{reset}"
            )
        if emoji:
            parts.append(emoji)
    ...
    def log(self, message: str, level: LogLevel = LogLevel.INFO):
        ...
        print(formatted)
        self.filesmanager.write_to_file(
            formatted, filename=self.log_file, filetype="logs", ext="log", mode="a"
        )
    ...
    def write_segment(self, segment: str, color_code: str):
        for char in segment:
            sys.stdout.write(
                (color_code if self.colored else "") + char + Style.RESET_ALL
            )
            sys.stdout.flush()
            if char != " ":
                time.sleep(0.001)

    def create_ascii_art_from_file(self, ascii_art_key: str = "generic"):
        self.__hide_cursor()
        # We need space before and after the art
        print("\n\n")
        default_ascii_key = self.ASCII_ART_FILE_MAP["generic"]
        ascii_art_key = ascii_art_key.lower()

        ascii_art_filepath = self.ASCII_ART_FILE_MAP.get(
            ascii_art_key, default_ascii_key
        )

        default_color = Fore.MAGENTA if self.colored else ""

        # This is a token replacement regex; it will find all instances of {token}
        # and match tokens like {A}, {B}, etc. to apply colors.
        # w+ - matches one or more word characters (letters, digits, underscores)
        # {token} - the token to replace
        token_remplacement = re.compile(r"\{(\w+)\}")

        current_color = default_color

        lines = self.filesmanager.read_file(Path(ascii_art_filepath))

        try:
            for line in lines:
                pos = 0
                for m in token_remplacement.finditer(line):
                    self.write_segment(line[pos : m.start()], current_color)
                    token_id = m.group(1)
                    current_color = (
                        SectionColors[token_id].value if self.colored else ""
                    )
                    pos = m.end()

                self.write_segment(line[pos:], current_color)
                sys.stdout.write("\n")
                sys.stdout.flush()
        finally:
            print("\n\n")
    ...
    def section(self, title: str):
        ...
        print(f"\n{color}▶ {title}{reset}")
        self.filesmanager.write_to_file(
            f"\n▶ {title}", filename=self.log_file, filetype="logs", ext="log", mode="a"
        )
        self.indent_level += 1
    ...
    def divider(self, char: str = "─", length: Optional[int] = None):
        ...
        self.filesmanager.write_to_file(
            char * length, filename=self.log_file, filetype="logs", ext="log", mode="a"
        )
    ...
    def batch_progress(
        self, items: list, operation: Callable, description: str = "Processing"
    ):
        total = len(items)

        for i, item in enumerate(items, 1):
            self.progress_bar(
                i, total, prefix=f"{description}", suffix=f"({i}/{total})"
            )
...
class OperationTracker:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        if exc_type is None:
            self.logger.success(
                f"Completed: {self.operation_name} " f"(took {elapsed:.2f}s)"
            )
        else:
            self.logger.error(
                f"Failed: {self.operation_name} " f"(after {elapsed:.2f}s): {exc_val}"
            )
